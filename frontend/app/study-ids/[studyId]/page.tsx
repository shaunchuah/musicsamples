...
async function resolveStudyIdentifierId(name: string, token: string): Promise<number | null> {
  const response = await fetch(
    buildBackendUrl(`/api/v3/study-ids/search/?query=${encodeURIComponent(name)}&page_size=5`),
    {
      headers: {
  ...
  const match = results.find(
    (entry: { id: number; name?: string }) => entry.name?.toLowerCase() === normalized,
  );
...
async function fetchStudyIdDetailById(
  ...
  const response = await fetch(buildBackendUrl(`/api/v3/study-ids/${studyId}/`), {
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    },
      ...
      {history.map((entry) => (
        ...
        >
          <p className="text-[0.7rem] text-muted-foreground">
            {formatDateTime(entry.history_date)} · {entry.history_user ?? "Unknown user"}
          </p>
          {entry.changes.length ? (
            <ul className="mt-2 space-y-1">
              {entry.changes.map((change) => (
                <li key={`${entry.history_date}-${change.field}`}>
                  <span className="font-semibold">{change.field.replace(/_/g, " ")}</span> changed
                  from <span className="font-semibold">{change.old ?? "blank field"}</span> to{" "}
                  <span className="font-semibold">{change.new ?? "blank field"}</span>
                </li>
              ))}
            </ul>
          ) : (
            <p className="mt-2 text-sm text-muted-foreground">No fields changed.</p>
          )}
        </div>
      ))}
    </div>
      ...
      <CardContent className="overflow-hidden">
        ...
        {actions ?? (
          <Button asChild size="sm">
            <Link href={`/study_id/edit/`}>Edit legacy</Link>
          </Button>
        )}
      </CardContent>
    </Card>
...
export default async function StudyIdDetailPage({ params }: { params: PageParams }) {
