...
import { ACCESS_TOKEN_MAX_AGE, AUTH_COOKIE_NAME, REFRESH_COOKIE_MAX_AGE, REFRESH_COOKIE_NAME, buildBackendUrl } from "@/lib/auth";
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!accessToken || !refreshToken) {
    return NextResponse.json({ error: "Authentication tokens missing from backend response." }, { status: 502 });
  }
