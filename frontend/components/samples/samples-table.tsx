...
import { Archive, ArrowDown, ArrowUp, ArrowUpDown, Download, Edit, Eye, Settings } from "lucide-react";
...
function PaginationControls({
	...
	return (
		<div className="flex flex-col gap-2 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
			<p>
				Showing {firstItemIndex === 0 ? 0 : firstItemIndex}-{lastItemIndex} of{" "}
				{totalLabel} samples
			</p>
...
export function SamplesTable() {
		...
		async function loadSamples() {
			...
			try {
				...
				const response = await fetch(
					`/api/dashboard/samples?${query.toString()}`,
					{
				...
				const totalPages =
					resolvedCount > 0
						? Math.ceil(resolvedCount / pagination.pageSize)
						: 1;
		...
		return [
			{
				header: ({ column }) => <SortableHeader column={column} title="Study" />,
					...
					enableSorting: true,
					cell: ({ row }) =>
						formatMaybe(row.original.study_name_label ?? row.original.study_name),
				},
			{
				header: ({ column }) => (
					<SortableHeader column={column} title="Sample ID" />
				),
			...
			},
			{
				id: "study_identifier",
					header: ({ column }) => (
						<SortableHeader column={column} title="Study ID" />
					),
			...
			{
					header: ({ column }) => (
						<SortableHeader column={column} title="Sublocation" />
					),
			...
			{
					header: ({ column }) => (
						<SortableHeader column={column} title="Sample Type" />
					),
					accessorKey: "sample_type",
					enableSorting: true,
					cell: ({ row }) =>
						formatMaybe(
							row.original.sample_type_label ?? row.original.sample_type,
						),
				},
			{
				header: ({ column }) => (
					<SortableHeader column={column} title="Collected At" />
				),
	...
	const lastItemIndex =
		rows.length > 0 ? pageIndex * pageSize + rows.length : 0;
			...
			<div className="flex items-center justify-between">
				...
				<DropdownMenu>
					...
					<DropdownMenuContent align="end">
						<DropdownMenuCheckboxItem
							checked={
								table.getColumn("timepoint_label")?.getIsVisible() ?? false
							}
						...
						<DropdownMenuCheckboxItem
							checked={
								table.getColumn("study_group_label")?.getIsVisible() ?? false
							}
							onCheckedChange={(value) =>
						...
						<DropdownMenuCheckboxItem
							...
							onCheckedChange={(value) =>
								table.getColumn("age")?.toggleVisibility(value)
							}
						>
							Age
						</DropdownMenuCheckboxItem>
						<DropdownMenuCheckboxItem
							...
							onCheckedChange={(value) =>
								table.getColumn("sex_label")?.toggleVisibility(value)
							}
						>
							Sex
						</DropdownMenuCheckboxItem>
						<DropdownMenuCheckboxItem
							checked={
								table.getColumn("study_center_label")?.getIsVisible() ?? false
							}
							onCheckedChange={(value) =>
						...
						<DropdownMenuCheckboxItem
							...
							onCheckedChange={(value) =>
								table.getColumn("crp")?.toggleVisibility(value)
							}
						>
							CRP
						</DropdownMenuCheckboxItem>
						...
						<DropdownMenuCheckboxItem
							checked={
								table.getColumn("genotype_data_available")?.getIsVisible() ??
								false
							}
							onCheckedChange={(value) =>
								table
									.getColumn("genotype_data_available")
									?.toggleVisibility(value)
							}
						>
							Genotyping
