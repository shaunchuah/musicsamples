...
function resolveAccessLevel(
	...
	if (
		groups.some(
			(group) =>
				typeof group === "string" && group.toLowerCase().includes("staff"),
		)
	) {
		return "staff";
	}
...
export function NavUser({ user }: NavUserProps) {
	...
	const accessLevel =
		combinedUser.accessLevel ?? resolveAccessLevel(combinedUser);
					...
					<DropdownMenuTrigger asChild>
						...
						>
							<Avatar className="h-8 w-8 rounded-lg">
								<AvatarImage
									src={combinedUser.avatar ?? undefined}
									alt={name}
								/>
