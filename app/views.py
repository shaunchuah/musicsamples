...
@login_required(login_url="/login/")
def paraffin_fix(request):
    import re

    sample_list = Sample.objects.filter(sample_type="paraffin_block")
    modified_count = 0
    total_analysed = 0
    pattern = r"^[A-E](?=\s)|(?<=\s)[A-E](?=\s)|[A-E](?=\.)|^[A-E]$|(?<=\s)[A-E]$"
    for sample in sample_list:
        if sample.paraffin_block_key is None:
            ...
            if key_value is not None:
                sample.paraffin_block_key = key_value.group()
                sample.save()
                modified_count += 1
    ...
    messages.success(
        request,
        f"""Paraffin block key added. {modified_count} samples modified. {total_analysed} samples analysed.""",
    )
    return redirect(reverse("paraffin_list"))


@login_required(login_url="/login/")
def paraffin_list(request):
    # Home Page
    sample_list = Sample.objects.filter(sample_type="paraffin_block")
    sample_count = sample_list.count()
    page = request.GET.get("page", 1)
    ...
    return render(request, "paraffin.html", context)
